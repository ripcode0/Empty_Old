...
GLenum GLTransform::GetVertextFormat(VertexFormat format)
{
	switch (format)
	{
...
GLenum GLTransform::GetTopology(Topology topology)
{
	switch (topology)
	{
...
GLenum GLTransform::GetInternalFormat(Format format)
{
	switch (format)
	{
...
GLenum GLTransform::GetFormat(Format format)
{
	switch (format)
	{
	case Format::FORMAT_RGBA32_UINT:
...
DXGI_FORMAT DXTransform::GetVertexFormat(VertexFormat format, uint formatSize)
{
	switch (formatSize)
	{
	case 1:
...
D3D_PRIMITIVE_TOPOLOGY DXTransform::GetTopology(Topology topology)
{
	switch (topology)
	{
	case Topology::POINTS:
...
DXGI_FORMAT DXTransform::GetFormat(Format format)
{
	switch (format)
	{
	case Format::FORMAT_RGBA32_UINT:
